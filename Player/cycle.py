...
from .FarmUpgrade import do_farm_upgrade
import subprocess
...
def do_cycle(appium_service, ui_locations: UILocations):
    now = time.time()
    next_image = take_screenshot()
    if now - last_done["research"] >= cycle_events["research"]:
        do_research_action(appium_service, ui_locations)
    ...
    elif now - last_done["shipping"] >= cycle_events["shipping"]:
        do_shipping_action(appium_service, ui_locations)
        last_done["shipping"] = now
    elif now - last_done["farmUpgrade"] >= cycle_events["farmUpgrade"]:
        do_farm_upgrade(appium_service, ui_locations)
        last_done["farmUpgrade"] = now
    elif now - last_done["chickenRun"] >= cycle_events["chickenRun"]:
        do_action(
            next_image,
            appium_service,
            ui_locations.ui_components[UIComponents.HatchBar],
            ui_locations.ui_components[UIComponents.ChickenRunButton],
        )
...
def run_player(appium_service: AppiumService):
    ...
    if welcome_back_accept_button != None:
        ...
        time.sleep(2)
    ui_locations = UILocations(appium_service)

    while True:
        do_cycle(appium_service, ui_locations)
        time.sleep(1)
